...
fn test_load_bom_hourly_fixture() {
    ...
    assert!(
        result.is_ok(),
        "Failed to deserialize BOM hourly forecast: {:?}",
        result.err()
    );
    ...
    assert!(
        first.temp.value > -50.0 && first.temp.value < 60.0,
        "Temperature should be in reasonable range"
    );
    assert!(
        first.wind.speed_kilometre < 500,
...
fn test_load_bom_daily_fixture() {
    ...
    assert!(
        result.is_ok(),
        "Failed to deserialize BOM daily forecast: {:?}",
        result.err()
    );
    ...
    if let Some(temp_max) = &first.temp_max {
        assert!(
            temp_max.value > -50.0 && temp_max.value < 60.0,
...
fn test_bom_hourly_fields() {
    ...
    for forecast in &response.data {
        ...
        assert!(
            forecast.temp.value.is_finite(),
            "Temperature should be finite"
        );
        assert!(
            forecast.temp_feels_like.value.is_finite(),
            "Apparent temp should be finite"
        );
...
fn test_bom_hourly_time_ordering() {
    ...
    assert!(
        response.data.len() > 1,
        "Should have multiple forecast hours"
    );
