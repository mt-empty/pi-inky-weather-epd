...
fn test_load_open_meteo_fixture() {
    ...
    assert!(result.is_ok(), "Failed to deserialize Open-Meteo forecast: {:?}", result.err());
    
    let response = result.unwrap();
    
    // Validate hourly data
    assert!(response.hourly.time.len() > 0, "Expected at least one hourly forecast entry");
    assert_eq!(response.hourly.time.len(), response.hourly.temperature_2m.len(), 
               "All hourly arrays should have same length");
    
    // Validate daily data
    assert!(response.daily.time.len() > 0, "Expected at least one daily forecast entry");
    assert_eq!(response.daily.time.len(), response.daily.temperature_2m_max.len(),
...
fn test_open_meteo_hourly_fields() {
    ...
    for i in 0..hourly.time.len() {
        ...
        assert!(temp > -50.0 && temp < 60.0, "Temperature should be in reasonable range");
        
        let apparent_temp = hourly.apparent_temperature[i];
        assert!(apparent_temp.is_finite(), "Apparent temperature should be finite");
        
        // Verify precipitation probability
        let precip_prob = hourly.precipitation_probability[i];
        assert!(precip_prob <= 100, "Precipitation probability should be <= 100%");
        
        // Verify precipitation amount
        let precip = hourly.precipitation[i];
        assert!(precip >= 0.0 && precip < 500.0, "Precipitation should be reasonable");
        ...
        assert!(wind >= 0.0 && wind < 500.0, "Wind speed should be reasonable");
...
fn test_open_meteo_daily_fields() {
    ...
    for i in 0..daily.time.len() {
        ...
        assert!(temp_max > -50.0 && temp_max < 60.0, "Max temperature should be in reasonable range");
        
        let temp_min = daily.temperature_2m_min[i];
        assert!(temp_min > -50.0 && temp_min < 60.0, "Min temperature should be in reasonable range");
        
        // Max should be >= min
        assert!(temp_max >= temp_min, "Max temperature should be >= min temperature");
        
        // Verify precipitation
        let precip_sum = daily.precipitation_sum[i];
        assert!(precip_sum >= 0.0 && precip_sum < 500.0, "Precipitation sum should be reasonable");
